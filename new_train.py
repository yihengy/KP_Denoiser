...
def main():
    ...
    for epoch in range(args.epochs):
        ...
        for i, data in enumerate(loader_train, 0):
            ...
            o_kernel = model(noise.float().to(args.device))
            
            batch_loss = criterion(o_kernel.to(args.device), noise.to(args.device), truth.to(args.device), args.outKerSize, args.patchSize).to(args.device)
            batch_loss.backward()
            MyOptim.step()
            model.eval()
        ...
        for i, data in enumerate(val_train, 0):
            ...
            v_kernel = model(val_noise.unsqueeze(1).float().to(args.device), args.outKerSize)
